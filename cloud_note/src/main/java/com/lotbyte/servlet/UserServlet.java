...
import com.lotbyte.inter.ThrowingPredicate;
...
public class UserServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // Optinal 高级用法  这里需要你琢磨琢磨 哈哈哈
        ap.filter(a -> StringUtils.isBlank(a)).map(ThrowingFunction.functionWrapper((a) -> {
            // 进入登录页面
            response.sendRedirect("login.jsp");
            return null;
        }, Exception.class))
                .orElseGet(ThrowingSupplier.supplierWrapper(() -> {
                    return ap.filter((a) -> a.equals("login")).map(ThrowingFunction.functionWrapper((a) -> {
                        // 用户登录
                        userLogin(request, response);
                        return null;
                    }, Exception.class))
                            .orElseGet(() -> {
                                return ap.filter((a) -> a.equals("userCenter")).map(ThrowingFunction.functionWrapper((a) -> {
                                    // 进入个人中心页面
                                    userCenter(request, response);
                                    return null;
                                }, Exception.class))
                                        .orElseGet(() -> {
                                            return ap.filter((a) -> a.equals("userHead")).map(ThrowingFunction.functionWrapper((a) -> {
                                                // 加载头像
                                                userHead(request, response);
                                                return null;
                                            }, Exception.class))
                                                    .orElseGet(() -> {
                                                        return ap.filter((a) -> a.equals("checkNick")).map(ThrowingFunction.functionWrapper((a) -> {
                                                            // 验证昵称的唯一性
                                                            checkNick(request, response);
                                                            return null;
                                                        }, Exception.class)).orElseGet(() -> {
                                                            return ap.filter((a) -> a.equals("updateInfo")).map(ThrowingFunction.functionWrapper((a) -> {
                                                                // 修改用户信息
                                                                updateInfo(request, response);
                                                                return null;
                                                            }, Exception.class))
                                                                    .orElseGet((ThrowingSupplier.supplierWrapper(()->{
                                                                       return ap.filter((a)->a.equals("logout")).map(ThrowingFunction.functionWrapper((a)->{
                                                                            // 用户退出登录
     ...
     */
    private void updateInfo(HttpServletRequest request, HttpServletResponse response) throws
        ...
        Optional.ofNullable(resultInfo).filter(r->r.getCode()==1).ifPresent(r->{
            request.getSession().setAttribute("user", resultInfo.getResult());
        });
    ...
    private void userLogin(HttpServletRequest request, HttpServletResponse response) {
        ...
        Optional.ofNullable(resultInfo).filter(ri->ri.getCode()==1).map(ThrowingFunction.functionWrapper((r)->{
            // 将用户对象存到session域中
            request.getSession().setAttribute("user", r.getResult());
            // 判断是否记住密码
            String rem = request.getParameter("rem");
            Optional.ofNullable(rem).filter((rm)->rm.equals(1)).ifPresent((rm)->{
                // 存cookie
            ...
            return  null;
        },Exception.class)).orElse(ThrowingSupplier.supplierWrapper(()->{
            // 设置request域对象
            request.setAttribute("resultInfo", resultInfo);
            // 请求转发到登录页面
