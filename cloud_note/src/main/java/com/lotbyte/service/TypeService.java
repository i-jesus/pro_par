...
public class TypeService {
	...
	public ResultInfo findNoteTypeList(Integer userId) {
		...
		if (!noteTypeList.isPresent()) { // 说明当前用户没有类型集合
			resultInfo.setCode(0);
			...
			resultInfo.setResult(noteTypeList.get());
		}
		return resultInfo;
	}
	...
	public ResultInfo addOrUpdate(Integer userId, String typeName, String typeId) {
		...
		int row = typeRepository.addOrUpdate(userId, typeName, typeId);
		if (row < 1) {
			resultInfo.setCode(0);
			resultInfo.setMsg("添加失败");
			return resultInfo;
	...
	public ResultInfo deleteType(String typeId) {
		...
		int row = typeRepository.deleteType(typeId);
